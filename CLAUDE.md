...
1. **Captura** — `CameraCapture` (OpenCV) lee frames de la webcam; `Dashboard` renderiza overlay en vivo (último veredicto, FPS, stats, costo estimado). ROI opcional: si `roi.enabled`, se selecciona una región interactiva al inicio (`cam.select_roi`). **Reconexión:** si en vivo `cam.read()` devuelve `None` (webcam desconectada), el loop no termina — `_reconnect_camera()` en `main.py` muestra "CAMARA DESCONECTADA", reintenta `cam.try_reopen()` con backoff (0.5→5 s) y sigue capturando al reconectar; Q sale durante la caída.
...
- `Alerter` (`src/alerter.py`) dispara **solo** cuando `verdict.status == "FAIL" and verdict.is_confirmed` (o PASS si `sound_on_pass`). Todas las alertas corren en threads daemon y tragan sus excepciones — una alerta fallida nunca afecta la inspección. El webhook (`_send_webhook`) postea payload compatible Slack/Discord (`{"text", "content"}`) a `WEBHOOK_URL`, enriquecido con el nombre del perfil activo y cada defecto (severidad, descripción, ubicación, confianza); el worker le pasa `analyzer.profile["name"]` a `alert_fail`.
- `Storage` comparte una conexión SQLite entre threads (`check_same_thread=False` + Lock). Dos tablas: `sessions` y `inspections` (incluye `raw_response`: el JSON crudo del veredicto final; `model`: qué modelo lo produjo; `primary_response`: el raw del modelo primario cuando hubo escalado híbrido — todo para auditoría). `_migrate()` agrega columnas nuevas a DBs de versiones previas con `ALTER TABLE` (idempotente vía `PRAGMA table_info`). Guardar el frame de evidencia depende de `storage.save_{pass,warn,fail}_frames`; va a `data/sessions/<id>/frames/`, los reportes HTML standalone a `reports/`. **Retención:** `storage.retention_days` (default 0 = infinito); al construir `Storage` (arranque de cualquier modo), `_apply_retention()` borra las sesiones —filas de `sessions`+`inspections` y la carpeta `data/sessions/<id>/`— cuyo `started_at` sea más viejo que N días. El borrado de disco es best-effort (`shutil.rmtree(ignore_errors=True)`): un fallo de FS no aborta el arranque.
...
- El dashboard usa solo ASCII en overlays: las fuentes Hershey de cv2 no renderizan unicode (✓✗⚠ saldrían como "?"). El panel de estado muestra además la racha del debounce (`consecutive_count/debounce_frames`) y el umbral `FAIL >= <fail_on_severity>` del perfil activo; `run_live` los pasa a `Dashboard.render` vía kwargs `debounce_frames`/`fail_on_severity`.
- Empaquetado: `pyproject.toml` (setuptools) expone el comando de consola `visual-qc` (= `main:main`) con `pip install -e .`; extras `[observability]` y `[dev]`. `requirements.txt` se mantiene para el CI.
