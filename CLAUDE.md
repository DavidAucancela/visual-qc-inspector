...
- Manejar `anthropic.RateLimitError` y `anthropic.APIStatusError` con las clases tipadas del SDK — un error de API no debe tumbar el loop de captura; registrar el frame como no-analizado y continuar.
...
- `Storage` comparte una conexión SQLite entre threads (`check_same_thread=False` + Lock). Dos tablas: `sessions` y `inspections`. Guardar el frame de evidencia depende de `storage.save_{pass,warn,fail}_frames`; va a `data/sessions/<id>/frames/`, los reportes HTML standalone a `reports/`.
